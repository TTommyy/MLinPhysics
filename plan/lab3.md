# LBVH Broad-Phase for NumPy Engine (2D)

## Scope and constraints

- Rebuild BVH every step; handle all colliders (balls + obstacles).
- 2D arrays only: positions, AABBs, and bounds are `(N, 2)`.
...
- Broad-phase: Linear BVH (LBVH) built from Morton codes (Karras 2012).
  - Compute per-entity AABBs and centers; normalize to world bounds; encode 2D Morton codes.
...
- AABBs: `aabb_min: (N, 2)`, `aabb_max: (N, 2)` computed from positions + half-extents per type.
- Tree (M = 2N-1 nodes):
  - `left, right, parent: (M,) int32` (−1 for leaf child absent)
  - `node_min, node_max: (M, 2) float64`
...
1. Gather colliders and compute AABBs (vectorized):
   ...
   - RECTANGLE_OBSTACLE: half = (w/2, h/2).
   - CIRCLE_OBSTACLE: same as BALL, half = r.

2. Morton encoding: normalize centers to `[0,1]^2` using simulation bounds; interleave bits into a 2D Morton code.
...
- Memory: M ≈ 2N nodes; roughly a few MB at N=50k.
